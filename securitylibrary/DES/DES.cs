...
namespace SecurityLibrary.DES
{
    ...
    public class DES : CryptographicTechnique
    {
        public override string Decrypt(string cipherText, string key)
        {
            ...
            string Permutated_Cipher_Text = "";
            for (int i = 0; i < IP.Length; i++)
            {
                int bit_indx = IP[i] - 1;
                Permutated_Cipher_Text += Binary_CipherText[bit_indx];
            }

            List<string> Ls = new List<string>();
            List<string> Rs = new List<string>();

            string L_0 = Permutated_Cipher_Text.Substring(0, 32);
            string R_0 = Permutated_Cipher_Text.Substring(32, 32);
            Ls.Add(L_0);
            Rs.Add(R_0);
            for (int x = 0; x < 16; x++)
            {
            ...
            string plainText = Convert.ToInt64(final, 2).ToString("X");
